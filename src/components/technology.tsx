...
const Technology = () => {
  ...
  return (
    <div className="flex justify-center bg-[#F9FAFC] max-md:px-4">
      <div className="flex flex-col items-center container mt-20 mb-28">
        <h1
          className={`text-4xl lg:text-[40px] font-bold text-[#0D0D2B] ${rubik.className}`}
        >
          Technology
        </h1>
        <span className="text-xl lg:text-[27px] font-semibold text-[#0D0D2B] mt-3 text-center">
          Developed By Izichain, To Become A General Tool For The Developer
          Community
        </span>
        <div className="flex flex-wrap max-md:flex-col justify-between mt-[48px] md:mt-[26px] -ml-[30px] lg:-ml-[45px] w-[calc(100%+30px)] lg:w-[calc(100%+45px)]">
          {data.map((item) => (
            <div
              key={item.title}
              className={`flex flex-col items-center pt-12 pb-8 px-6 rounded-2xl shadow-[0px_3px_10px_-3px_#39177733] md:w-[calc(33.3%-30px)] lg:w-[calc(33.3%-45px)] ml-[30px] lg:ml-[45px] mt-5 md:mt-[38px] ${
